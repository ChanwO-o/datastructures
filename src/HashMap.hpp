...
private:
    ...
    Node** nodes;
    unsigned int currentBucketSize;
       
    void nullFill();

    int modBySize(const int hashResult) const;
    
    int getIndex(const string& key) const;
    
    Node* getNode(const string& key, const int index) const;
    
    void addWithoutRehash(const string& key, const string& value);

    void deleteLinkList(Node* node);

    int listLength(const Node* root) const;
